...
class APIAdapter(GetInfo):
    ...
    def connect(self):
        ...
        try:
            geo_res = requests.get(self.openstreetmap_url, params={'q': 'London', 'format': 'json', 'limit': 1}, headers=headers, timeout=5)
            sky_res = requests.get(self.opensky_url, params={'lamin': 0, 'lamax': 1, 'lomin': 0, 'lomax': 1}, timeout=5)
            if geo_res.status_code == 200 and sky_res.status_code == 200:
                print("Успешное подключение к API")
                return True
            print(f"Один из сервисов вернул ошибку. Nominatim: {geo_res.status_code}, OpenSky: {sky_res.status_code}")
            return False
